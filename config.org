...
No longer friends with catppuccin, moving to Misterioso.
This one is a built-in and is pretty good.
...
~project-vc-merge-submodules~ being ~nil~ avoid including submodules.
And we add the custom function as a fallback.
...
#+begin_src emacs-lisp
    ...
    ("C-c p t"   . treemacs-display-current-project-exclusively)
...
High Level Shading Language for shaders. Fun stuff.

#+begin_src emacs-lisp
  (use-package hlsl-mode
    :ensure (hlsl-mode :type git :host github :repo "jcaw/hlsl-mode"))
#+end_src
