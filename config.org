...
#+begin_src emacs-lisp
  (defun kr-buffer-permissions ()
    "Show the buffer permissions for the current buffer"
    (cond ((and buffer-read-only (buffer-modified-p)) (propertize " !! " 'face (kr-inverted-color 'font-lock-builtin-face)))
          (buffer-read-only (propertize " RO " 'face (kr-inverted-color font-lock-string-face)))
          ((buffer-modified-p) (propertize " <> " 'face (kr-inverted-color 'font-lock-number-face)))
          (t (propertize " RW " 'face (kr-inverted-color 'font-lock-keyword-face)))))

  (defun kr-header-line ()
    (let* ((left (concat (kr-buffer-permissions)
                        " " (make-string (recursion-depth) ?\[)
                        (or (buffer-file-name) (buffer-name))
                        " " (make-string (recursion-depth) ?\])))
           (right (concat (propertize (format-time-string " %F ") 'face (kr-inverted-color 'font-lock-string-face))))
           (pad-width (- (window-total-width)
                         (+ (length left)
                            (length right)))))
      (concat left (make-string pad-width ?\s) right)))


  (use-package emacs
    :custom
    (header-line-format '(:eval (kr-header-line))))
#+end_src
